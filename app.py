...
def load_db():
    ...
    try:
        ...
        # Make sure required keys exist and seed default data
        # when the database contains empty collections.
        if not db.get("mandis"):
        ...
        if "bookings" not in db or not isinstance(db["bookings"], list):
            db["bookings"] = []

        # Persist the seeded defaults so the API works immediately.
        save_db(db)
