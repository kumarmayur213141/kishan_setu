        ...
        <div class="token-card">
            ...
            <div class="token-grid">
                ...
                <div class="token-item">
                    ...
                    <strong>${formatCurrency(booking.dbt.amount)}</strong>
                </div>
    ...
    if (currentIndex >= 0 && currentIndex < statuses.length - 1) {
        ...
        html += `
            <div class="workflow-action">
                <button
                    class="primary-btn next-status-btn"
                    onclick="advanceStatus('${booking.booking_id}', '${nextStatus}', '${booking.token}')"
                >
                    ▶️ Next Step: ${nextStatus}
                </button>
                <small>Prototype Demo: button dabane se status next stage par jayega.</small>
            </div>
        `;

    } else if (booking.status === "DBT Paid") {

        html += `
            <div class="workflow-complete">
                🎉 <b>Procurement workflow complete.</b> DBT payment marked as Paid.
            </div>
        `;
    }
...
async function advanceStatus(bookingId, nextStatus, token) {
    ...
    try {
        ...
        if (!response.ok || !result.success) {
            alert(result.message || "Status update nahi ho saka.");
            return;
        }
        ...
        console.error(error);
        alert("Status update ke liye server se connection nahi ho raha.");
...
function setupVoice() {
    ...
    button.addEventListener("click", () => {
        ...
        if (!("speechSynthesis" in window)) {
            alert("Is browser mein Awaaz feature supported nahi hai.");
            return;
        }
        ...
        const text = `
            KisanSetu किसान खरीद पोर्टल।
            अपनी फसल बेचने के लिए मंडी, फसल, मात्रा, तारीख और समय स्लॉट चुनें।
            स्लॉट बुक करने के बाद आपको डिजिटल टोकन मिलेगा।
            टोकन से लाइव स्टेटस देख सकते हैं।
        `;
